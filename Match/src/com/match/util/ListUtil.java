...
public class ListUtil {
	 ...
	 */
	public static <T> List<T> shufflePartialList(List<T> list, int startIndex, int endIndex) throws ArrayIndexOutOfBoundsException {
	 ...
	 * @throws ArrayIndexOutOfBoundsException
	 */
	protected static <T> List<T> swap(List<T> list, int firstSwapIndex, int secondSwapIndex) throws ArrayIndexOutOfBoundsException {
		
		int len = list.size();
		int index;
		if((index = firstSwapIndex) < 0 || (index = secondSwapIndex) >= len || (index = secondSwapIndex) < 0 || (index = firstSwapIndex) >= len) {
			throw new ArrayIndexOutOfBoundsException(index);
		} else {
